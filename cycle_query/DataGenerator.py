...
class MainGenerator:
	...
	def getNumberOfEdges(self, database):
		if self.relationCardinality == "Full":
			return self.edgesForEachDatabase[database]
		elif self.relationCardinality == "Random":
	...
	def getDistributionOfEdges(self, available, atLeastDifferent):
		if (self.connectionPattern == "Random"):
			...
			while (available > 0):
				...
				iterations += 1
			return result
		elif (self.connectionPattern == "HardCase"):
			...
			while (available > 0):
				...
				if (iterations < atLeastDifferent):
					...
					choose_from = max_degree
				if (choose_from > 0):
					chosen = random.randint(1, choose_from)
					if (iterations < atLeastDifferent):
						result[iterations] += chosen
					else:
						result.append(chosen)
	...
	def build_database(self):
		...
		for k in range(self.databasesNo):
			...
			if (self.connectionPattern != "HardCase"): min_values_of_Attibute0 = 0
			else:
